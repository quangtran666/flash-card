...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Tạo bộ thẻ mới</h1>
        </div>
            ...
            <form action="{{ route('decks.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name"
                           ...
                           placeholder="Nhập tên bộ thẻ" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-4 @error('description') border-red-500 @enderror"
                              placeholder="Nhập mô tả cho bộ thẻ">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="language" id="language"
                            ...
                            <option value="english" {{ old('language') === 'english' ? 'selected' : '' }}>Tiếng Anh</option>
                            <option value="vietnamese" {{ old('language') === 'vietnamese' ? 'selected' : '' }}>Tiếng Việt</option>
                            <option value="japanese" {{ old('language') === 'japanese' ? 'selected' : '' }}>Tiếng Nhật</option>
                            <option value="korean" {{ old('language') === 'korean' ? 'selected' : '' }}>Tiếng Hàn</option>
                            <option value="french" {{ old('language') === 'french' ? 'selected' : '' }}>Tiếng Pháp</option>
                            <option value="chinese" {{ old('language') === 'chinese' ? 'selected' : '' }}>Tiếng Trung</option>
                        </select>
                        @error('language')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
