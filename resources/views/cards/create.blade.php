...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Thêm thẻ mới - {{ $deck->name }}</h1>
        </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="front_content" id="front_content"
                                   ...
                                   placeholder="Nhập từ vựng" value="{{ old('front_content') }}" required>
                            @error('front_content')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="pronunciation" id="pronunciation"
                                   ...
                                   placeholder="Nhập phiên âm (nếu có)" value="{{ old('pronunciation') }}">
                            @error('pronunciation')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="url" name="image_url" id="image_url"
                                   ...
                                   placeholder="Nhập đường dẫn hình ảnh" value="{{ old('image_url') }}">
                            @error('image_url')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="back_content" id="back_content"
                                   ...
                                   placeholder="Nhập định nghĩa hoặc nghĩa của từ" value="{{ old('back_content') }}" required>
                            @error('back_content')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="example" id="example" rows="4"
                                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-4 @error('example') border-red-500 @enderror"
                                      placeholder="Nhập ví dụ sử dụng">{{ old('example') }}</textarea>
                            @error('example')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex justify-center">
                        <div class="w-64 h-40 bg-white border-2 border-gray-300 rounded-lg shadow-md p-4 flex items-center justify-center">
                            <div class="text-center">
                                <p class="text-xl font-bold text-gray-800" id="preview-front">Từ vựng</p>
                                <p class="text-sm text-gray-500" id="preview-pronunciation">/phiên âm/</p>
                            </div>
                        </div>
                    </div>
                </div>
